...
final class Dtdr_Controller{
    ...
    public function add_settings_link( $link ){
        $settings[] = '<a href="' . admin_url( 'options-general.php?page=droit-dark-settings' ) . '" class="drdt-settings-plugin"> '.esc_html__('Settings', 'droit-dark').'</a>';
        
        if( class_exists('\DroitDark\Includes\Dtdr_Features') ){
            $settings[] = '<a href="' . admin_url( 'options-general.php?page=droit-dark-settings#droit_license' ) . '" class="drdt-settings-plugin"> '.esc_html__('License', 'droit-dark').'</a>';
        }
